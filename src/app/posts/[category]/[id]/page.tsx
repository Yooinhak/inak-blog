  ...
  <div className="mt-8 animate-pulse space-y-4">
    {Array.from({ length: 8 }).map((_, i) => (
      <div key={i} className="space-y-2">
        <div className="h-4 rounded bg-white/10" style={{ width: `${75 + ((i * 17) % 25)}%` }} />
        <div className="h-4 rounded bg-white/10" style={{ width: `${50 + ((i * 13) % 40)}%` }} />
      </div>
    ))}
  </div>
);
...
const Page = async ({ params }: PageParams) => {
  ...
  return (
    <section className="relative min-h-[calc(100vh_-_65px_-_52px)] overflow-hidden text-white">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/3 right-0 h-[360px] w-[360px] rounded-full bg-pink-600/30 blur-[140px]" />
        <div className="absolute -bottom-32 left-1/2 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-cyan-500/20 blur-[160px]" />
      </div>

      <div className="relative z-10 w-full px-0 py-12">
        <div className="mx-auto w-full max-w-4xl rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl sm:p-10">
          <div className="prose prose-invert w-full max-w-none">
            ...
            <div className="not-prose space-y-3">
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-white/60">{category}</p>
